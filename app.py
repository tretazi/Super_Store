...
<style>
    ...
    header[data-testid="stHeader"] {
        ...
        border-bottom: 1px solid #2a3042 !important;
    }
    ...
    header[data-testid="stHeader"] button svg { fill: #c9d1e0 !important; }
    ...
    .charts-row-2b { grid-template-columns: 2fr 3fr; }
    @media (max-width: 768px) {
        .charts-row-2,
        .charts-row-2b { grid-template-columns: 1fr; }
    }
    ...
    /* Responsive table: hide less important cols on small screens */
    @media (max-width: 500px) {
        .top-flop-table .col-sales,
        .top-flop-table .col-margin { display: none; }
        .top-flop-table { font-size: 0.75rem; }
    }

    .badge-top {
        background: #0d2818; color: #34d399;
        border: 1px solid #34d399; border-radius: 4px;
        padding: 2px 7px; font-size: 0.68rem; font-weight: 600;
        font-family: 'DM Mono', monospace;
    }
    .badge-flop {
    ...
    /* ── STREAMLIT COLUMNS: override for smaller screens ── */
    @media (max-width: 768px) {
        [data-testid="column"] {
            min-width: 100% !important;
            flex: 1 1 100% !important;
        }
        /* Stack st.columns on mobile */
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown(
        "<div style='font-size:0.72rem;color:#3d4d6b;text-align:center;font-family:DM Mono'>"
        "Superstore Dashboard<br/>Portfolio — Data Analyst</div>",
        unsafe_allow_html=True
    )
...
if len(sel_years) >= 2:
    ...
    yoy_str, yoy_cls = "— sélectionne 2+ années", "neutral"
...
with col1:
    ...
    fig.add_trace(go.Bar(x=yearly["Year"], y=yearly["Sales"],  name="CA",     marker_color=ACCENT1, opacity=0.85), secondary_y=False)
    fig.add_trace(go.Bar(x=yearly["Year"], y=yearly["Profit"], name="Profit", marker_color=ACCENT2, opacity=0.85), secondary_y=False)
    fig.add_trace(go.Scatter(x=yearly["Year"], y=yearly["Marge %"], name="Marge %",
                             mode="lines+markers", line=dict(color=ACCENT3, width=2), marker=dict(size=7)), secondary_y=True)
...
with col2:
    ...
    fig2.add_trace(go.Bar(
        y=regional["Region"], x=regional["Sales"], orientation="h",
        marker_color=colors, opacity=0.85,
        text=[f"${v:,.0f}" for v in regional["Sales"]],
        textposition="inside", textfont=dict(size=10, color="white"), name="CA"
    ))
...
with col3:
    ...
    fig3 = px.pie(cat_data, values="Sales", names="Category",
                  color_discrete_sequence=[ACCENT1, ACCENT2, ACCENT3], hole=0.55)
...
with col4:
    ...
    fig4 = go.Figure(go.Bar(
        y=sub_data["Sub-Category"], x=sub_data["Profit"], orientation="h",
...
def render_table(data, badge_class, badge_label):
    ...
    for _, r in data.iterrows():
        name = r["Product Name"][:38] + "…" if len(r["Product Name"]) > 38 else r["Product Name"]
        ...
        <tr>
            ...
            <td>{name}</td>
            <td class="col-sales"  style="text-align:right">${r['Sales']:,.0f}</td>
            <td class="{profit_cls}"       style="text-align:right">{profit_sign}${r['Profit']:,.0f}</td>
            <td class="col-margin" style="text-align:right">{r['Marge']:.1%}</td>
        </tr>"""
    return rows

# Wrap tables in responsive HTML grid
st.markdown(f"""
<div class="tables-grid">
  <div>
    <table class="top-flop-table">
      <thead><tr><th></th><th>Produit</th><th class="col-sales" style="text-align:right">CA</th><th style="text-align:right">Profit</th><th class="col-margin" style="text-align:right">Marge</th></tr></thead>
...
fig5.add_trace(go.Scatter(
    x=monthly["Month"], y=monthly["Sales"], mode="lines", name="CA mensuel",
    line=dict(color=ACCENT1, width=1.5), opacity=0.5,
    fill="tozeroy", fillcolor="rgba(99,102,241,0.08)"
))
fig5.add_trace(go.Scatter(
    x=monthly["Month"], y=monthly["MA3"], mode="lines", name="Moyenne mobile 3 mois",
...
# ── FOOTER ────────────────────────────────────────────────────────────────────
st.markdown("""
<div style="text-align:center;padding:1.5rem 0 0.5rem;color:#3d4d6b;
            font-size:0.72rem;font-family:'DM Mono',monospace;
            border-top:1px solid #2a3042;margin-top:1rem">
